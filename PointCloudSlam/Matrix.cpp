...
void Matrix::setDiag(double s, int32_t i1, int32_t i2) {
	if (i2 == -1) i2 = (int32_t)std::fmin((float)m - 1, (float)n - 1);
	for (int32_t i = i1; i <= i2; i++)
		val[i][i] = s;
}
...
void Matrix::eye() {
	for (int32_t i = 0; i < m; i++)
		for (int32_t j = 0; j < n; j++)
			val[i][j] = 0;
	for (int32_t i = 0; i < std::fmin((float)m, (float)n); i++)
		val[i][i] = 1;
}
...
void Matrix::svd(Matrix &U2, Matrix &W, Matrix &V) {
	...
	W = Matrix((int32_t)std::fmin((float)m, (float)n), 1, w);

	// extract mxm submatrix U
	U2.setMat(U.getMat((int32_t)0, (int32_t)0, m - 1, (int32_t)std::fmin((float)m - 1, (float)n - 1)), 0, 0);
