...
#pragma unroll
	...
	for (int i = 0; i < BODIES_CHUNK; i++){
		...
		in_pos_temp_distances = cm_sqrt(((in_pos_model.select<BODIES_CHUNK, ELEMS_BODY>(0) - query_temp_x)*(in_pos_model.select<BODIES_CHUNK, ELEMS_BODY>(0) - query_temp_x)) + ((in_pos_model.select<BODIES_CHUNK, ELEMS_BODY>(1) - query_temp_y)*(in_pos_model.select<BODIES_CHUNK, ELEMS_BODY>(1) - query_temp_y)) + ((in_pos_model.select<BODIES_CHUNK, ELEMS_BODY>(2) - query_temp_z)*(in_pos_model.select<BODIES_CHUNK, ELEMS_BODY>(2) - query_temp_z)));
		...
		if (query_temp_x != 0.0f && query_temp_y != 0.0f && query_temp_z != 0.0f && ((query_temp_x - match_model_x)*match_model_normal_x + (query_temp_y - match_model_y)*match_model_normal_y + (query_temp_z - match_model_z)*match_model_normal_z)>10){
			...
			//least_squares_vector_2(h + 3) = (match_model_normal_x*(match_model_x - query_temp_x) + (match_model_normal_y*(match_model_y - query_temp_y)) + (match_model_normal_z*(match_model_z - query_temp_z)));
