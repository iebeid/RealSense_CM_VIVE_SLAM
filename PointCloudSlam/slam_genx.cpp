...
#pragma unroll
	...
	for (int i = 0; i < BODIES_CHUNK; i++){
		...
		if (query_temp_x != 0.0f && query_temp_y != 0.0f && query_temp_z != 0.0f && ((query_temp_x - match_model_x)*match_model_normal_x + (query_temp_y - match_model_y)*match_model_normal_y + (query_temp_z - match_model_z)*match_model_normal_z)>0.1){
...
//	vector<float, BODIES_CHUNK*ELEMS_BODY> least_squares_vector_2;
//
//	matrix<float, float, BODIES_CHUNK, ELEMS_BODY> in_pos_model_matrix;
//	matrix<float, float, BODIES_CHUNK, ELEMS_BODY> in_normal_model_matrix;
//	matrix<float, float, BODIES_CHUNK, ELEMS_BODY> in_pos_temp_matrix;
//
//	in_pos_model_matrix.row(0) = in_pos_model.select<float, BODIES_CHUNK>(0);
//	in_pos_model_matrix.row(1) = in_pos_model.select<float, BODIES_CHUNK>(1);
//	in_pos_model_matrix.row(2) = in_pos_model.select<float, BODIES_CHUNK>(2);
//	in_pos_model_matrix.row(3) = in_pos_model.select<float, BODIES_CHUNK>(3);
//
//	in_normal_model_matrix.row(0) = in_normal_model.select<float, BODIES_CHUNK>(0);
//	in_normal_model_matrix.row(1) = in_normal_model.select<float, BODIES_CHUNK>(1);
//	in_normal_model_matrix.row(2) = in_normal_model.select<float, BODIES_CHUNK>(2);
//	in_normal_model_matrix.row(3) = in_normal_model.select<float, BODIES_CHUNK>(3);
//
//	in_pos_temp_matrix.row(0) = in_pos_temp.select<float, BODIES_CHUNK>(0);
//	in_pos_temp_matrix.row(1) = in_pos_temp.select<float, BODIES_CHUNK>(1);
//	in_pos_temp_matrix.row(2) = in_pos_temp.select<float, BODIES_CHUNK>(2);
//	in_pos_temp_matrix.row(3) = in_pos_temp.select<float, BODIES_CHUNK>(3);
//
//
//#pragma unroll
//	for (int i = 0; i < BODIES_CHUNK; i += BODIES_PER_RW)
