...
GLFWwindow* setup_vr(int width, int height, int pos_x, int pos_y, const char * title){
	...
	//ar_window = setup_window(fb_width, fb_height, 100, 100, "Augmented Reality");
	...
	//fov = device->QueryColorFieldOfView();
	//FPS f_c;
...
void render_window_ar(const GLvoid * image, int width, int height, int frame_buffer_width, int frame_buffer_height, GLFWwindow* window, float fov)
{
	...
	glBlitFramebuffer(0, 0, frame_buffer_width, frame_buffer_height, 0, 0, frame_buffer_width, frame_buffer_height, GL_COLOR_BUFFER_BIT, GL_LINEAR);
...
void render_vive(const GLvoid * image){
	...
	//while (pSenseManager->AcquireFrame(true) >= PXC_STATUS_NO_ERROR) {
		...
		//
		render_window_ar(image, windowWidth, windowHeight, recommended_fb_width, recommended_fb_height, ar_window, fov.y);
